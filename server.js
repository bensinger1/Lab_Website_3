...
app.get('/team_stats', function(req,res) {
  ...
  db.task('get-everything', task=> {
    return task.batch([task.any(games), task.any(wins), task.any(losses)]);
  })
  ...
  .then(stats => {
    res.render('pages/team_stats',{
      my_title: "Team Stats",
      team_stats: stats[0],
      wins: stats[1][0].count,
      losses: stats[2][0].count
    })
  })
  .catch(error => {
    req.flash('error',err);
...
app.get('/player_info/post', function(req,res){
  ...
  var info = "SELECT * FROM football_players WHERE name='"+player+"';";
  var games = "SELECT COUNT(*) FROM football_games WHERE (SELECT id FROM football_players WHERE name = '"+player+"')= any(players);"

  db.task('get-everything', task=> {
    return task.batch([task.any(names), task.any(info),task.any(games)]);
  })
  ...
  .then(player_info => {
    res.render('pages/player_info',{
      ...
      info: player_info[1][0],
      games: player_info[2][0].count
    })
  })
  .catch(error => {
    req.flash('error',err);
